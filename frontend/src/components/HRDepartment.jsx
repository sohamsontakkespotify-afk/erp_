    ...
    switch (modalType) {
      ...
      case 'editJobPosting':
        ...
        try {
          ...
          const response = await fetch(`${API_BASE}/hr/job-postings/${jobForm.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
      ...
      case 'scheduleInterview':
        if (!interviewForm.applicationId || !interviewForm.scheduledDate || !interviewForm.scheduledTime || !interviewForm.interviewType) {
          toast({ title: "Missing Information", description: "Application, date, time, and interview type are required.", variant: "destructive" });
          return;
        }
        ...
        try {
          const payload = {
            ...
            duration_minutes: interviewForm.duration_minutes || 60,
      ...
      case 'screenApplication':
        if (!applicationForm.screeningDecision) {
          toast({ title: "Missing Information", description: "Please select a screening decision.", variant: "destructive" });
          return;
        }

        try {
          const response = await fetch(`${API_BASE}/hr/job-applications/${selectedApplication.id}/status`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              status: applicationForm.screeningDecision === 'pass' ? 'screening_passed' : 'rejected',
              screeningNotes: applicationForm.screeningNotes,
              reviewerId: user?.id || 1
            })
          });

          if (!response.ok) throw new Error('Failed to update screening status');

          toast({ title: "Success", description: "Screening status updated successfully" });
          closeModal();
          fetchJobApplications();
        } catch (err) {
          toast({ title: "Error", description: err.message, variant: "destructive" });
      ...
      case 'updateInterviewResult':
        if (!interviewForm.decision) {
          toast({ title: "Missing Information", description: "Please select an interview decision.", variant: "destructive" });
          return;
        }

        try {
          await handleUpdateInterviewResult(
            selectedInterview.id,
            'completed',
            interviewForm.feedback,
            interviewForm.rating ? parseFloat(interviewForm.rating) : null,
            interviewForm.decision
          );
        } catch (err) {
          toast({ title: "Error", description: err.message, variant: "destructive" });
        }
      ...
      case 'setJoining':
        if (!applicationForm.joiningDate) {
          toast({ title: "Missing Information", description: "Please select a joining date.", variant: "destructive" });
          return;
        }

        try {
          await handleSetJoiningDetails(
            selectedApplication.id,
            applicationForm.joiningDate,
            applicationForm.documentsSubmitted
          );
        } catch (err) {
          toast({ title: "Error", description: err.message, variant: "destructive" });
        }
...
const DashboardView = ({ employees = [] }) => {  // ✅ Accept employees as prop (default to empty array)
  ...
  const AttendanceView = () => {
    ...
    const rejectedLeaves = leaveRequests.filter(l => l.status === 'rejected').length;
      ...
      const filteredLeaveRequests = leaveRequests.filter(leave => {
        ...
        const searchTerms = leaveSearchQuery.toLowerCase().trim().split(/\s+/);
    ...
    const filteredLeaveBalances = leaveBalances.filter(balance => {
      if (!balanceSearchQuery.trim()) return true;
      ...
      const searchTerms = balanceSearchQuery.toLowerCase().trim().split(/\s+/);
    ...
    const resetLeaveRequests = () => {
      if (window.confirm("Are you sure you want to reset all leave requests from view?")) {
        setLeaveRequests([]);
      }
    };

    const resetLeaveBalances = () => {
      if (window.confirm("Are you sure you want to reset all leave balances to 0?")) {
        setLeaveBalances(prev =>
          prev.map(balance => ({
            ...balance,
            casualLeave: { ...balance.casualLeave, used: 0 },
            sickLeave: { ...balance.sickLeave, used: 0 },
            earnedLeave: { ...balance.earnedLeave, used: 0 },
          }))
        );
      }
    };
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-red-600">{employees.length-presentCount}</span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {leaveRequests.length > 0 ? filteredLeaveRequests.map((leave) => {
                return (
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {leaveBalances.length > 0 ? filteredLeaveBalances.map((balance) => {
                const employee = employees.find(e => e.id === balance.employeeId);
                return (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {payrolls.length > 0 ? payrolls.map((payroll) => {
                  ...
                  const salaryDisplay = payroll.salaryType === 'hourly' 
                    ...
                    : `₹${(payroll.monthlySalary || 0).toLocaleString()}`;
                  ...
                  return (
                    <tr key={payroll.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{(payroll.allowances || 0).toLocaleString()}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{(payroll.deductions || 0).toLocaleString()}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{(payroll.grossSalary || 0).toLocaleString()}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-600">₹{(payroll.netSalary || 0).toLocaleString()}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
  ...
  const RecruitmentView = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          <div className="p-4 border-b border-gray-200">
            <h3 className="text-black font-semibold">Active Job Postings</h3>
          </div>
              ...
              <div key={job.id} className="border border-gray-200 rounded-lg p-4">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-1">Posted: {job.createdAt ? new Date(job.createdAt).toLocaleDateString() : 'N/A'} • Applications: {job.applications || 0}</p>
                  </div>
                  <div className="flex space-x-2">
                    <button
                      onClick={() => {
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          <div className="p-4 border-b border-gray-200">
            <h3 className="text-black font-semibold">Applications Management</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          <div className="p-4 border-b border-gray-200 flex justify-between items-center">
            <h3 className="text-black font-semibold">Interview Management</h3>
            <Button
              onClick={() => {
                setInterviewForm({
                  applicationId: '',
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {interviews.length > 0 ? interviews.map((interview) => (
                  <tr key={interview.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{interview.applicantName || interview.candidateName}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 capitalize">{interview.interviewType || 'N/A'}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {interview.decision ? (
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full capitalize ${
                          interview.decision === 'selected' ? 'bg-green-100 text-green-800' :
                          interview.decision === 'rejected' ? 'bg-red-100 text-red-800' :
                          'bg-yellow-100 text-yellow-800'
                        }`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex gap-2">
                        ...
                        {(interview.status === 'scheduled' || interview.decision === 'on_hold') && (interview.decision !== 'selected' && interview.decision !== 'rejected') && (
                          <button 
                            onClick={() => {
                              setSelectedInterview(interview);
                              setInterviewForm({
                  ...
                  <tr>
                    <td colSpan="7" className="px-6 py-4 text-center text-gray-500">No interviews scheduled</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col bg-white border-gray-200 text-gray-900">
            <DialogHeader className="flex-shrink-0">
              <DialogTitle className="text-gray-900">
                ...
                {modalType === 'scheduleInterview' && (selectedApplication ? `Schedule Interview – ${selectedApplication.candidateName || selectedApplication.applicantName}` : 'Schedule Interview')}
                {modalType === 'viewInterview' && 'Interview Details'}
                {modalType === 'updateInterviewResult' && (selectedInterview ? `Update Interview Result – ${selectedInterview.candidateName}` : 'Update Interview Result')}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{modalFormData.employeeId || '—'}</p>
                  </div>
                  <div>
                    <Label>Status</Label>
                  ...
                  <div>
                    <Label>Name</Label>
                    <p className="text-sm text-gray-600">{`${modalFormData.firstName ?? ''} ${modalFormData.lastName ?? ''}`.trim() || '—'}</p>
                  </div>
                  <div>
                    <Label>Email</Label>
                    <p className="text-sm text-gray-600">{modalFormData.email || '—'}</p>
                  </div>
                  <div>
                    <Label>Phone</Label>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{modalFormData.department || '—'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{modalFormData.joiningDate || '—'}</p>
                  </div>
                  <div>
                    <Label>Salary</Label>
                    <p className="text-sm text-gray-600">
                      {modalFormData.salary !== undefined && modalFormData.salary !== ''
                        ? `₹${Number(modalFormData.salary).toLocaleString()}`
                        : '—'}
                    </p>
                  </div>
                  <div>
    ...
    <div>
      ...
      />
           ...
           {/* View Job Posting Modal */}
            {modalType === 'viewJobPosting' && selectedEmployee && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>Job Title</Label>
                    <p className="text-gray-900 font-medium mt-1">{selectedEmployee.title}</p>
                  </div>
                  <div>
                    <Label>Department</Label>
                    <p className="text-gray-900 font-medium mt-1">{selectedEmployee.department}</p>
                  </div>
                  <div>
                    <Label>Location</Label>
                    <p className="text-gray-900 font-medium mt-1">{selectedEmployee.location || 'Not specified'}</p>
                  </div>
                  <div>
                    <Label>Employment Type</Label>
                    <p className="text-gray-900 font-medium mt-1 capitalize">{selectedEmployee.employmentType?.replace('-', ' ') || 'Not specified'}</p>
                  </div>
                  <div>
                    <Label>Experience Level</Label>
                    <p className="text-gray-900 font-medium mt-1 capitalize">{selectedEmployee.experienceLevel || 'Not specified'}</p>
                  </div>
                  <div>
                    <Label>Salary Range</Label>
                    <p className="text-gray-900 font-medium mt-1">{selectedEmployee.salaryRange || 'Not specified'}</p>
                  </div>
                  <div>
                    <Label>Status</Label>
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full mt-1 ${
                      selectedEmployee.status === 'open' ? 'bg-green-100 text-green-800' :
                      selectedEmployee.status === 'closed' ? 'bg-red-100 text-red-800' :
                      selectedEmployee.status === 'filled' ? 'bg-blue-100 text-blue-800' :
                      'bg-gray-100 text-gray-800'
                    }`}>
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 font-medium mt-1">{selectedEmployee.createdAt ? new Date(selectedEmployee.createdAt).toLocaleDateString() : 'N/A'}</p>
                  </div>
                </div>
                <div>
                  <Label>Job Description</Label>
                  <p className="text-gray-900 mt-1 whitespace-pre-wrap">{selectedEmployee.description}</p>
                </div>
                {selectedEmployee.requirements && (
                  <div>
                    <Label>Requirements</Label>
                    <p className="text-gray-900 mt-1 whitespace-pre-wrap">{selectedEmployee.requirements}</p>
                  </div>
                )}
                {selectedEmployee.responsibilities && (
                  <div>
                    <Label>Responsibilities</Label>
                    <p className="text-gray-900 mt-1 whitespace-pre-wrap">{selectedEmployee.responsibilities}</p>
                  </div>
                )}
                {selectedEmployee.applicationDeadline && (
                  <div>
                    <Label>Application Deadline</Label>
                    <p className="text-gray-900 font-medium mt-1">{new Date(selectedEmployee.applicationDeadline).toLocaleDateString()}</p>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">₹{Number(selectedApplication.expectedSalary || 0).toLocaleString()}</p>
                  </div>
                  <div>
              ...
              {/* Candidate/Application Selector */}
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      const selectedApp = jobApplications.find(app => app.id.toString() === value);
                      if (selectedApp) {
                        setInterviewForm({
                          ...interviewForm,
                          applicationId: value,
                          candidateName: selectedApp.candidateName || selectedApp.applicantName || '',
                  ...
                  >
                    ...
                    <SelectContent>
                      {jobApplications && jobApplications.length > 0 ? (
                        jobApplications
                          .filter(app => ['submitted','screening_passed', 'interviewed', 'shortlisted'].includes(app.status))
                          .length > 0 ? (
                            jobApplications
                              .filter(app => ['submitted','screening_passed', 'interviewed', 'shortlisted'].includes(app.status))
                              .map(app => (
                                <SelectItem key={app.id} value={app.id.toString()}>
                                  {app.candidateName || app.applicantName || 'Unknown'} - {app.jobTitle || 'No Job Title'} ({app.status})
                                </SelectItem>
                              ))
                          ) : (
                            <div className="px-2 py-1.5 text-sm text-gray-500">
                              No eligible candidates. Applications must have status: screening_passed, interviewed, or shortlisted.
                            </div>
                          )
                      ) : (
                        <div className="px-2 py-1.5 text-sm text-gray-500">No applications available</div>
                      )}
                    </SelectContent>
                  </Select>
                  {jobApplications && jobApplications.length > 0 && (
                    <p className="text-xs text-gray-500 mt-1">
                      ...
                      Eligible: {jobApplications.filter(app => ['submitted','screening_passed', 'interviewed', 'shortlisted'].includes(app.status)).length}
                    </p>
                  )}
                </div>


                {/* Candidate Info Card - Only show when candidate is selected */}
            ...
            {modalType === 'generatePayroll' && (
              <div className="space-y-4">
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-700 mt-1">
                        <strong>Monthly Employees:</strong> Salary = (Monthly Salary ÷ Working Days) × Attended Days. Working days exclude Sundays.<br/>
                        <strong>Hourly Employees:</strong> Salary = Hourly Rate × Total Hours Worked. Deduction: 5% of gross salary.<br/>
                        <strong>Monthly Deductions:</strong> PF (12% of min(base, ₹15,000)), ESI (0.75% if gross &lt; ₹21,000), PT (₹200 fixed), IT (5% if gross &gt; ₹50,000).
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {employees.filter(emp => emp.status === 'active').map((emp) => (
                            <SelectItem key={emp.id} value={emp.id.toString()}>
                              {emp.name || `${emp.firstName} ${emp.lastName}`} - {emp.department} (₹{emp.salary?.toLocaleString()})
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                ...
                {generatePayrollMode === 'bulk' && (
                  <>
                    <div>
                      <Label htmlFor="bulkAllowances">Default Allowances for All (₹)</Label>
                      <Input
                        id="bulkAllowances"
                        type="number"
                        ...
                        {payrollForm.salaryType === 'hourly' ? (
                          <div className="space-y-2">
                            <div className="flex justify-between text-xs text-gray-500 pb-2 border-b">
                              <span>Hourly Rate: ₹{parseFloat(payrollForm.basicSalary || 0).toLocaleString()}</span>
                              <span>Total Hours: {attendanceData.totalHours}</span>
                            </div>
                            <div className="flex justify-between text-sm border-t pt-2">
                              <span className="text-gray-600">Gross Salary (Rate × Hours):</span>
                              <span className="font-medium text-purple-600">
                                ₹{(attendanceData.totalHours * parseFloat(payrollForm.basicSalary || 0)).toLocaleString(undefined, {maximumFractionDigits: 2})}
                              </span>
                            </div>
                            ...
                            {payrollForm.allowances && parseFloat(payrollForm.allowances) > 0 && (
                              <div className="flex justify-between text-xs text-gray-500 pt-2 border-t">
                                <span>Note: Allowances (₹{parseFloat(payrollForm.allowances || 0).toLocaleString()}) are tracked separately for hourly employees</span>
                              </div>
                            )}
                          </div>
                        ) : (
                          <div className="space-y-2">
                            <div className="flex justify-between text-xs text-gray-500 pb-2 border-b">
                              <span>Monthly Salary: ₹{parseFloat(payrollForm.basicSalary || 0).toLocaleString()}</span>
                              <span>Daily Rate: ₹{attendanceData.totalWorkingDays > 0 ? (parseFloat(payrollForm.basicSalary || 0) / attendanceData.totalWorkingDays).toFixed(2) : '0'}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                            ...
                            {payrollForm.allowances && (
                              <div className="flex justify-between text-sm">
                                ...
                                <span className="font-medium text-green-600">+ ₹{parseFloat(payrollForm.allowances || 0).toLocaleString()}</span>
                              </div>
                            )}
                            <div className="flex justify-between text-sm border-t pt-2">
                            ...
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="font-medium text-red-600">
                                - ₹{attendanceData.totalWorkingDays > 0 ? (() => { const baseSalary = (parseFloat(payrollForm.basicSalary || 0) / attendanceData.totalWorkingDays) * attendanceData.attendedDays; const gross = baseSalary + parseFloat(payrollForm.allowances || 0); const pf = Math.min(baseSalary, 15000) * 0.12; const esi = gross < 21000 ? gross * 0.0075 : 0; const pt = 200; const it = gross > 50000 ? gross * 0.05 : 0; return (pf + esi + pt + it).toLocaleString(undefined, {maximumFractionDigits: 2}); })() : '0'}
                              </span>
                            </div>
                            <div className="flex justify-between text-base font-bold border-t pt-2 bg-green-50 px-2 py-1 rounded">
                              <span className="text-gray-900">Net Salary:</span>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-700 mt-1">
                        Update allowances or pay period dates. Salary is calculated based on attendance: (Monthly Salary ÷ Working Days) × Attended Days.
                ...
                <div className="border-t border-gray-200 pt-4">
                  ...
                  <div className="space-y-2 bg-white rounded-lg p-3 border border-gray-200">
                    <div className="flex justify-between py-2 border-b border-gray-200">
                      ...
                      <span className="text-sm font-medium text-gray-900">₹{(selectedPayroll.monthlySalary || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-200">
                      <span className="text-sm text-gray-700">Allowances</span>
                      <span className="text-sm font-medium text-green-600">+ ₹{(selectedPayroll.allowances || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-200">
                      <span className="text-sm font-medium text-gray-900">Gross Salary</span>
                      <span className="text-sm font-semibold text-gray-900">₹{(selectedPayroll.grossSalary || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-200">
                      <span className="text-sm text-gray-700">Deductions</span>
                      <span className="text-sm font-medium text-red-600">- ₹{(selectedPayroll.deductions || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-3 bg-green-50 border border-green-200 px-3 rounded-lg mt-2">
                      ...
                      <span className="text-base font-bold text-green-600">₹{(selectedPayroll.netSalary || 0).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
